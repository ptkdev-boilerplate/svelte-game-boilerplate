...
const play = (): void => {
	...
	const map = addLevel(
		...
		],
		{
			...
			b: () => [sprite("wall_botright"), area({ width: 4, offset: vec2(12, 0) }), solid()],
			c: () => [sprite("wall_topleft"), area(), solid()],
			d: () => [sprite("wall_topright"), area(), solid()],
			w: () => [sprite("wall"), area(), solid()],
			t: () => [sprite("wall_top"), area({ height: 4, offset: vec2(0, 12) }), solid()],
			l: () => [sprite("wall_left"), area({ width: 4 }), solid()],
			r: () => [sprite("wall_right"), area({ width: 4, offset: vec2(12, 0) }), solid()],
		},
	);
	...
	const sword = add([pos(), sprite("sword"), rotate(0), follow(player, vec2(-4, 9)), spin()]);
